//
// Created by darik on 4/28/2020.
//

#include "catch.hpp"
#include "../src/AVLTree.hpp"

TEST_CASE("AVLTree insertion", "[algorithms]") {
    {
        AVLTree<int> tree;
        tree.insert(59);
        tree.insert(1);
...
TEST_CASE("AVLTree search", "[algorithms]"){
    AVLTree<int> tree;
    for(int i = 0; i < 40; ++i){
        tree.insert(i);
    }
...
TEST_CASE("AVLTree iterators", "[algorithms]") {
    AVLTree<int> tree;
    for (int i = 0; i < 30; ++i) {
        tree.insert(i);
    }
...
TEST_CASE("AVLTree deletion", "[algorithms]") {
    {
        AVLTree<int> tree;
        for (int i = 0; i < 16; ++i) {
            tree.insert(i);
